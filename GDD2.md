...
- **Estrutura de upgrade — RESOLVIDO**: `UpgradeDefinition` (SO próprio), com `targetItem` (o `ItemDefinition` que ele bonifica) + `damageBonus` (soma flat). `PlayerUpgrades` (novo singleton) acumula o bônus por item comprado na run; reseta sozinho porque é estado de cena normal (não grava em `PlayerPrefs`).
- **Importante — raridade é dos UPGRADES, não dos itens**: os itens (Dinamite/Bomba/Espada/...) ficam **sempre visíveis** na loja, com custo e quantidade por compra fixos (`ItemDefinition.cost`/`stockPerPurchase`) — nada de sorteio aí. É a oferta de **upgrade** que é sorteada por raridade a cada visita.

### 9.1 Loja — itens fixos + upgrades por raridade — ✅ IMPLEMENTADO (v1, ainda placeholder visual)

- **Moeda** (`PlayerCurrency`, novo singleton): guarda `SkullCount`, separado do `DungeonGoldManager` (ouro é o que se defende, cabeça é o que se ganha e gasta). `EnemyAI` credita uma cabeça ao `PlayerCurrency` sempre que um esqueleto morre (`EnemyDefinition.minSkullReward`/`maxSkullReward`, fallback nos mesmos campos em `EnemyAI`), independente de estar carregando ouro roubado.
- **Itens fixos** (`ShopManager.availableItems` + `ShopItemSlotUI`): a mesma lista sempre visível, um slot por item, sem sorteio. Comprar chama `PlayerCurrency.TrySpend(item.cost)` e `PlayerInventory.AddStock(item, item.stockPerPurchase)` — os mesmos métodos que o `Debug Starting Item` já usava.
- **Raridade** (`ItemRarity`, novo enum): Comum, Incomum, Raro, Épico, Lendário, Mítico — agora um campo de `UpgradeDefinition`, não de `ItemDefinition`.
- **Chance por raridade** (`ShopRarityTable`, novo ScriptableObject): pesos por raridade somando sempre 100% (mesmo padrão de redistribuição de `EnemyAI.zones`/`ItemDefinition.variants`). `ShopRarityTable.PickRarity()` sorteia por esses pesos.
- **Progressão** (`ShopManager.progression`, array de `WaveRarityTable`): cada entrada associa uma `ShopRarityTable` a "a partir de qual wave" ela vale — dá a progressão "raridade sobe com o jogo avançando" sem fórmula, só configurando os assets pelo Inspector (mesmo espírito de `SpawnManager.waves`).
- **Cor por raridade** (`RarityPalette`, novo ScriptableObject): mapeia cada `ItemRarity` pra uma cor, usada no fundo do slot de upgrade.
- **Aplicação do bônus**: `IThrowable` ganhou `SetSourceItem(ItemDefinition)` — o `SlingshotController` avisa o projétil, na hora de lançar, de qual item da loja ele veio; `Projectile_Bomb.ApplyDamage` soma `PlayerUpgrades.GetDamageBonus(sourceItem)` ao dano base antes de aplicar o multiplicador de zona.
- **Fluxo**: `SpawnManager.RunWaves()` chama `ShopManager.Open(upcomingWave)` assim que uma wave termina (antes do `WaitForSeconds(timeBetweenWaves)`) e `ShopManager.Close()` antes de liberar a próxima — a loja ocupa exatamente a mesma pausa fixa que já existia, sem alterar o ritmo. `Open()` preenche os slots de item (fixos) e sorteia N ofertas de upgrade (`ShopManager.upgradeSlots`, um `ShopUpgradeSlotUI` cada) dentre `ShopManager.availableUpgrades`.
- **UI**: tudo por Canvas (World Space, mesmo padrão de `StartGameCanvas`/`GameOverCanvas` — `TrackedDeviceGraphicRaycaster`, funciona com o laser de VR já existente), posicionado no lugar da relíquia antiga (`GemObjective`, que ficou órfã desde que o ouro virou a condição de derrota — script não foi tocado, só reaproveitado visualmente). Sem balcão/vendedor de verdade ainda — isso é visão futura (ver nota no fim desta seção).
- **Ideia registrada, não implementada — minigame de sorte**: usar o próprio estilingue (só com pedra, travando bomba/itens durante a compra) num mini-jogo de 3 alvos (fixos e/ou móveis, estilo dummy de treino) entre waves — a quantidade de acertos influenciaria qual `ShopRarityTable` é usada no sorteio de upgrades daquela visita (mais acerto = tabela com mais chance de raridade alta). Combinado que isso entra **depois** da loja básica estar funcionando e testável, como uma camada em cima do `GetTableForWave` já existente.
- **Não incluído nesta rodada**: lista maior de itens além de Bomba/Pedra e de upgrades além dos de exemplo (pendência #10 continua aberta), número de slots de inventário (#11).
...
8. ~~Estrutura exata de dados dos upgrades~~ — **RESOLVIDO**: SO próprio (`UpgradeDefinition`, `targetItem` + `damageBonus`), efeito acumulado em `PlayerUpgrades` (não persiste, reseta a cada run).
9. ~~UI do inventário e da loja~~ — **RESOLVIDO (loja, v1)**: ver seção 9.1 — Canvas World Space, `ShopManager`/`ShopItemSlotUI`/`ShopUpgradeSlotUI`/`PlayerCurrency`/`PlayerUpgrades`/`ShopRarityTable`/`RarityPalette`. UI de inventário (troca de item equipado durante o combate) continua não desenhada.
